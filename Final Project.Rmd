...
ggplot(data = trends, mapping = aes(x = depression)) +
  geom_histogram(color = "dodgerblue", fill = "lightsteelblue", binwidth = 3)+
  facet_grid(case_cat~.) +
...
death_by_case <- aov(depression ~ case_cat, data = trends)
...
restriction_by_case <- aov(depression ~ restriction_cat, data = trends)
...
aCOVID_by_rate <- aov(anxiety ~ case_cat, data = trends)
...
a_restriction_by_case <- aov(anxiety ~ restriction_cat, data = trends)
...
The plots above show a positive correlation between both the new COVID-19 cases and the severity of restrictions with google searches for anxiety relative to state. However, we're yet to find out which variable have the stronger correlation. In other words, are people more anxious of new COVID cases or COVID-related restrictions? 
